...
- 🌐 **Navigateur** — n'importe quel site (YouTube, SoundCloud…) : titre,
  **miniature de la vidéo**, artiste, **barre de temps**, et un bouton « Watch on … ».
- 🖥️ **Applications Windows** — détecte l'appli au premier plan (FL Studio, VLC,
  Vocaloid, cmd…) avec son nom, le **projet/fichier ouvert**, et sa **vraie icône
  extraite du `.exe`**.
- 🎯 **Priorité automatique** — affiche l'appli ou l'onglet où tu es *actuellement*.
- 🎨 **Indicateur personnalisable** sur les pages : 6 positions, animations, sons,
  et **thèmes partageables** (JSON + CSS).
- ⚙️ **Tout se règle depuis l'extension** (aucun fichier à éditer) : ID Discord,
  indicateur, sites, thèmes.
- 🌍 **Interface FR / EN** + guide de démarrage intégré.
- 🔒 **100 % local & open source** — aucune collecte de données.

> 💡 Comme **PreMiD**, mais ça couvre **aussi tes applications PC**.
