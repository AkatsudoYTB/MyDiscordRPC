...
- 🌐 **Browser** — any site (YouTube, SoundCloud…): title, **video thumbnail**,
  artist, **progress bar**, and a “Watch on …” button.
- 🖥️ **Windows apps** — detects the foreground app (FL Studio, VLC, Vocaloid,
  cmd…) with its name, the **open project/file**, and its **real icon extracted
  from the `.exe`**.
- 🎯 **Automatic priority** — shows the app or tab you're *currently* on.
- 🎨 **Customizable on-page indicator**: 6 positions, animations, sounds, and
  **shareable themes** (JSON + CSS).
- ⚙️ **Everything is set from the extension** (no files to edit): Discord ID,
  indicator, sites, themes.
- 🌍 **English / French UI** + built-in getting-started guide.
- 🔒 **100 % local & open source** — no data collection.

> 💡 Like **PreMiD**, but it also covers **your desktop apps**.
